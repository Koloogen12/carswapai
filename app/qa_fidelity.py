...
MAP = [
    ...
    # Путь [1,0,1], а не [1,0]: [1,0] — это колонка канвы, и в ней рядом с
    # рамкой лежат подпись «05 Плёнка · основной экран» и рассуждение автора
    # «Машина занимает экран. Артикула нет в прайсе точки — его не существует:
    # штриховка вместо ложного выбора». Рассуждение `strip_rationale()` срезает,
    # но словарь самой рамки — двенадцать слов, а порог отсечения — двадцать,
    # и функция честно возвращалась к необрезанному варианту. Десять из
    # двадцати двух ожидаемых слов приходили из комментария к макету, продукт
    # вынести их не мог бы никогда, и маршрут годами стоял ровно на пороге:
    # 55%, любая правка формулировки роняла его в красное.
    #
    # [1,0,1] — сама рамка 390×790, двенадцать слов интерфейса, 100%. Это тот
    # же приём и по той же причине, что уже применён к /c/... ниже.
    ('/g/jetcar-mytishchi','03-phase3-client-garage',       2, [1, 0, 1]),
