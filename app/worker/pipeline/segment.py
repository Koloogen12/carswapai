...
def segment(img: np.ndarray) -> dict[str, np.ndarray]:
    ...
    Если обученные веса частей на месте — работает сеть, и всё, что ниже,
    не выполняется вовсе. Эвристики остаются запасным путём: они проверены
    на двенадцати реальных кадрах и признаны негодными для боя (крыша вместо
    окон, капот вместо колёс, дверная ручка вместо номера), но пустая маска
    хуже плохой — на ней не работает даже отказ по номеру.
    """
    from . import parts
    if parts.available():
        return parts.segment(img)
