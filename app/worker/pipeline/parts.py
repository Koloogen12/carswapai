...
def segment(img: np.ndarray) -> dict[str, np.ndarray]:
    ...
    if main is not None and (main > 0).any():
        ...
        masks['car_ref'] = main
    # ── ГОСНОМЕР: отдельный детектор ────────────────────────────────
    # Сеть частей его почти не находит — 1 кадр из 12, см. pipeline/plate.py.
    # Если специальный детектор доступен, его слово здесь главнее.
    from . import plate as plate_mod
    if plate_mod.available():
        found = plate_mod.detect(img)
        if (found > 0).any():
            masks['plate'] = found
