...
psql -h "$S" -p "$PORT" -U carswap_app -d carswap -v ON_ERROR_STOP=1 \
  -f tests/invariants.sql -f tests/queue.sql -f tests/consent-rolls.sql -f tests/operations.sql -f tests/renders-erasure.sql -f tests/channel-resolver.sql -f tests/offer-consent.sql \
